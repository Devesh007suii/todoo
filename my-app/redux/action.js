...
const serverUrl = "https://todoapp007.onrender.com/api/v1"
...
export const addTask = (title,description) => async (dispatch) => {
    try {
        ...
        const {data} = await axios.post(`${serverUrl}/newtask`,{
            title,
            description,
        },{
            header: {
...
export const resetPassword = (otp, newPassword) => async (dispatch) => {
    try {
        ...
        const {data} = await axios.put(`${serverUrl}/resetpassword`, {otp,newPassword},{
           headers: {
            "Content-Type": "application/json"
           }
        });
